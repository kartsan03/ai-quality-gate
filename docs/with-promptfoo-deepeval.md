...
aiqg ingest results.jsonl --out my_case/outputs --field response
# write my_case/source.txt and my_case/case.yml, then:
aiqg run my_case
```

If promptfoo's export nests the model output differently, adjust `--field`
(dotted path) or write a three-line script that maps each row onto
`outputs/NNNN.json`. Keep promptfoo for prompt iteration and red-teaming;
...
(out / "0000.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
```
